...
for i in range(0,len(prediction)):
    ...
    actual_lstm_predictions.append(pos)

# Compute Quadratic Weighted Kappa for test labels
w2v_lstm_qwk_score = quadratic_weighted_kappa(np.asarray(test_labels), actual_lstm_predictions)

print("W2V LSTM QWK: ", w2v_lstm_qwk_score)
...
result_rfc = forest.predict(answerTestVectors)

qwk_rfc_score = quadratic_weighted_kappa(test_labels, result)

print("RFC QWK: ", qwk_rfc_score)

final_svm_predictions = weighted_predictions(result_rfc, qwk_rfc_score)


# Averaging
sum_of_qwk = w2v_lstm_qwk_score + w2v_dnn_qwk_score + qwk_score_rbf_svc + qwk_rfc_score

final_predictions = []

for index in range(len(actual_dnn_predictions)):
    ...
    final_predictions.append(sum_of_values/sum_of_qwk)
